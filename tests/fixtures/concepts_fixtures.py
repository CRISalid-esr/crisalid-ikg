...
@pytest_asyncio.fixture(name="persisted_concept_pydantic_model")
async def fixture_persisted_concept_pydantic_model(concept_pydantic_model) -> Concept:
    """
    ...
    await concept_dao.create(concept_pydantic_model)
    return concept_pydantic_model
...
@pytest_asyncio.fixture(name="persisted_concept_without_uri_pydantic_model")
async def fixture_persisted_concept_without_uri_pydantic_model(
    ...
    factory = AbstractDAOFactory().get_dao_factory(get_app_settings().graph_db)
    concept_dao: ConceptDAO = factory.get_dao(Concept)
    await concept_dao.create(concept_without_uri_pydantic_model)
    return concept_without_uri_pydantic_model


@pytest_asyncio.fixture(name="concept_without_uri_pydantic_model")
async def fixture_concept_without_uri_pydantic_model(concept_without_uri_json_data) -> Concept:
    """
    Create a basic concept pydantic model
    :return: basic concept pydantic model
    """
    return _concept_from_json_data(concept_without_uri_json_data)
...
@pytest_asyncio.fixture(name="invalid_concept_without_uri_json_data")
async def fixture_invalid_concept_without_uri_json_data(_base_path) -> dict:
    """
    Create a basic person json data
    :return: basic person json data
    """
    return _concept_json_data_from_file(_base_path, "invalid_concept_without_uri")
