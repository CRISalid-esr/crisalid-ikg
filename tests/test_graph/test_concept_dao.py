...
async def test_create_concept(concept_pydantic_model: Concept):
    """
    ...
    await dao.create(concept_pydantic_model)
    concept_from_db = await dao.find_by_uri(concept_pydantic_model.uri)
    assert concept_from_db
    assert concept_from_db.uri == concept_pydantic_model.uri
    for pref_label in concept_pydantic_model.pref_labels:
    ...
    for alt_label in concept_pydantic_model.alt_labels:
