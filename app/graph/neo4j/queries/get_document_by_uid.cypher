...
WITH document, title, concepts, sr, contribution, contributor, pn,
     ...
     collect(DISTINCT CASE
       ...
       END) AS last_names
WITH document, title, concepts, sr, contribution,
     contributor,
     collect(DISTINCT CASE
...
WITH document, title, concepts, collect(DISTINCT contribution {. *, contributor:single_contributor}) AS contributions, sr
RETURN document,
