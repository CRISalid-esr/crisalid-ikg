...
class AMQPInterface:
    ...
    def __init__(self, settings: AppSettings):
        ...
        self.publications_queue: aio_pika.Queue | None = None
        ...
        self.inner_publications_tasks_queue: asyncio.Queue | None = None
        self.publications_message_processing_workers: list[asyncio.Task] | None = None
        self.keys = [self.settings.amqp_reference_event_routing_key]
    ...
    async def stop_listening(self) -> None:
        ...
        try:
            await asyncio.wait_for(
                self.inner_publications_tasks_queue.join(),
                timeout=self.settings.amqp_wait_before_shutdown,
            )
        finally:
            for worker in self.publications_message_processing_workers:
                worker.cancel()
            await self.pika_channel.close()
            await self.pika_connexion.close()

    async def _attach_message_processing_workers(self):
        self.publications_message_processing_workers = []
        self.inner_publications_tasks_queue = asyncio.Queue(maxsize=self.INNER_PUBLICATIONS_TASKS_QUEUE_LENGTH)
        for worker_id in range(self.settings.amqp_task_parallelism_limit):
            processor = await self._message_processor()
            self.publications_message_processing_workers.append(
                asyncio.create_task(
    ...
    async def _message_processor(self):
        return AMQPPublicationMessageProcessor(
            exchange=self.pika_exchange,
            tasks_queue=self.inner_publications_tasks_queue,
            settings=self.settings,
        )

    async def _listen_to_messages(self):
        async with self.publications_queue.iterator() as queue_iter:
            async for message in queue_iter:
                await self.inner_publications_tasks_queue.put(message)
    ...
    async def _bind_queue(self) -> None:
        ...
        self.publications_queue = await self.pika_channel.declare_queue(
            self.settings.amqp_publications_queue_name,
            durable=True,
            arguments={"x-consumer-timeout": self.settings.amqp_consumer_ack_timeout},
        )
        for key in self.keys:
            await self.publications_queue.bind(self.pika_exchange, routing_key=key)
